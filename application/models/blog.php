...
class Blog extends Model {
	...
	function getCatPosts($data){
		/* $cat_id = $this->getCatId($data['cat_slug']);
		//$cat_id = intval($cat_id[0]['category_id']);
		...
		$srch->setPageSize($data['pagesize']);
		$srch->addMultipleFields(array('bp.`post_id`', 'bp.`post_title`', 'bp.`post_view_count`', 'bp.`post_short_description`', 'bp.`post_content`', 'bp.`post_seo_name`', 'count(bpc.`comment_id`) as comment_count',  'bp.`post_comment_status`', 'bp.`post_published`', 'bpimg.`post_image_file_name`'));
		$srch->addOrder('bp.post_published', 'DESC');
		$srch->addGroupBy('bp.post_id');
		//echo $srch->getQuery();
		$rs = $srch->getResultSet();
	...
	function getSearchPost($data){
		...
		$srch->setPageNumber($data['page']);
		$srch->setPageSize($data['pagesize']);
		$srch->addMultipleFields(array('bp.`post_id`','bp.`post_view_count`', 'bp.`post_title`', 'bp.`post_short_description`', 'bp.`post_content`', 'bp.`post_seo_name`', 'count(bpc.`comment_id`) as comment_count',  'bp.`post_comment_status`', 'bp.`post_published`', 'bpimg.`post_image_file_name`'));
	...
	{	
		...
		$query = $this->db->query("SELECT GROUP_CONCAT(`category_id`) as cat_ids FROM `tbl_blog_post_categories` where `category_code` LIKE (CONCAT('',(select `category_code` from `tbl_blog_post_categories` where `category_seo_name` = '".$cat_slug."'),'%'))");
	...
	function getBlogPosts($data){
		...
		$srch->setPageNumber($data['page']);
		$srch->setPageSize($data['pagesize']);
		$srch->addMultipleFields(array('bp.`post_id`', 'bp.`post_title`', 'bp.`post_short_description`', 'bp.`post_content`', 'bp.`post_seo_name`', 'bp.`post_view_count`', 'count(bpc.`comment_id`) as comment_count',  'bp.`post_comment_status`', 'bp.`post_published`', 'bpimg.`post_image_file_name`', 'bp.`post_contibutor_name`'));
		$srch->addOrder('bp.post_published', 'DESC');
		$srch->addGroupBy('bp.post_id');
		//echo $srch->getQuery();
		$rs = $srch->getResultSet();
	...
	function getPostComments($data){
		...
		$srch->addMultipleFields(array('bpc.`comment_author_name`', 'bpc.`comment_content`', 'bpc.`comment_date_time`', 'u.`user_id`', 'u.`user_photo`'));
