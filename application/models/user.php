...
class User extends Model {
	...
	public function getCurrentOnlineWritersCount() {
		...
		//$srch->addCondition('user_last_activity', '>', 'DATE_SUB(NOW(), INTERVAL 60 SECOND)', 'AND', true);
	...
	public function getTopWriters($page = 1, $filter = '', $filter_val,$pagesize=15,$featured=true,$paper_id,$disp_id) {
				...
				break;
 				case 'pep_type' :
					if($filter_val > 0) {
						...
						$srch->addDirectCondition('FIND_IN_SET("'.$filter_val.'", task_paptype_id)');
					}
					$srch->addOrder('user_rating','DESC');
					$srch->addOrder('bid.orders_completed','ASC');
					break; 
		...
		if(intval($paper_id) > 0) {
			...
			$srch->addDirectCondition('FIND_IN_SET("'.$paper_id.'", task_paptype_id)');
