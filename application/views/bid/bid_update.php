    ...
    <div class="contentArea">
        ...
        <div class="fullWrapper">
			...
			<h2 class="pagetitle"><?php echo Utilities::getLabel( 'L_Order' ); ?>: #<?php echo $order['task_ref_id'];?></h2>
				...
				$tr->appendElement('td',array(),Utilities::getLabel( 'L_Status') . ': Bidding ');
				
				$tr->appendElement('td',array(),Utilities::getLabel( 'L_Pages') . ': '.$order['task_pages']);
				
				$tr->appendElement('td',array(), Utilities::getLabel( 'L_Total_Bids') . ': ' . Bid::getnumbids($order['task_id']));
				...
				echo $tbl->getHtml();
				$frmbid->addHiddenField('', 'bid_task_id', $order['task_id'], 'bid_task_id');
				$frmbid->addHiddenField('', 'bid_id', $order['bid_id'], 'bid_id');
				$frmbid->fill(array('bid_price'=>$order['bid_price']));
				echo $frmbid->getFormHtml();
				
					/* $price = $order['bid_price_per_page']*$order['task_pages']; */
					...
					/* $tr->appendElement('td',array(),'<a href="'.generateUrl("bid","bid_update",array($order['bid_id'])).'">CHANGE MY BID</a>',TRUE); */
