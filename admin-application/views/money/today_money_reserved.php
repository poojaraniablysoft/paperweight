...
//echo "<pre>".print_r($arr_listing,true)."</pre>";exit;
?>
<section class="rightPanel wide">
	<ul class="breadcrumb">
		...
		<li>Total Reserved Amount - 24 Hrs</li>
	</ul>
	<?php echo Message::getHtml(); ?>
	<div class="title"><h2>Total Reserved Amount - 24 Hrs : <span><?php echo CONF_CURRENCY.$total_reserved;?></span></h2></div>
						...
						switch ($key){
							...
							case 'Action':								
								if($row['task_status'] == 4 && $row['reserved_amount'] != 0) {
									$td->appendElement('a', array('class'=>'button small black','href'=>generateUrl('transactions','reserve_earnings',array($row['res_task_id'],$row['task_user_id']))),'Transfer to Earnings',true);
								}else {
