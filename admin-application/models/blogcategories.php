...
class Blogcategories extends Model {
	...
	function getAllCategories($blog_category_id=null){
	...
	function getBlogcategoriesData($data){
		...
		$srch->addMultipleFields(array('bpc.*', 'bpc1.`category_title` AS cat_parent'));
		...
		$srch->addOrder('bpc.category_id', 'DESC');
		//echo $srch->getQuery();
		$rs = $srch->getResultSet();
		$this->total_records = $srch->recordCount();
	...
	static function getBlogCategoryName($blog_category_id){
		$blog_category_id = intval($blog_category_id);
		if(intval($blog_category_id) == 0){
			return false;
		}
		$srch = new SearchBase('tbl_blog_post_categories', 'bp');
		$srch->addCondition('bp.category_id', '=', $blog_category_id);
		$srch->addFld('category_title');
		//echo $srch->getQuery();
		$rs = $srch->getResultSet();
		return $this->db->fetch($rs);
	}
