...
class Labels extends Model {
    ...
    function search($criteria) {
        ...
        foreach($criteria as $key=>$val) {
            ...
            case 'id':
                ...
                break;
			case 'keyword':
				$srch->addDirectCondition('(tlang.label_key LIKE '. $this->db->quoteVariable('%' . $val . '%') .' OR tlang.label_caption like '. $this->db->quoteVariable('%' . $val . '%') . ')' );
                break;
			case 'pagesize':
	...
	function addUpdateLabel($data){
		...
		$assign_fields['label_caption'] = $data['label_caption'];
	...
	function updateLabelText($data){
		...
		$assign_fields = array();
		if(!empty($data['value'])){
			$assign_fields[$arr_param[1]] = $data['value'];
			$record->assignValues($assign_fields);
			if($label_id === 0 && $record->addNew()){
				return $record->getId();
			}elseif($label_id > 0 && $record->update(array('smt'=>'label_id=?', 'vals'=>array($label_id)))){
