...
class Blogposts extends Model {
	...
	function addUpdate($data){
		...
		$assign_fields['post_contibutor_name'] = $data['post_contibutor_name'];
	...
	function getBlogpostsData($data){
		...
		//$srch->joinTable('tbl_blog_post_categories', 'LEFT OUTER JOIN', 'bc.`category_id` = bpcr.`relation_post_id`', 'bc');
		
		if(isset($data['post_category']) && intval($data['post_category']) > 0) {			
			$srch->addCondition('bpcr.relation_category_id', '=',  $data['post_category']);
		}
		...
		$srch->addMultipleFields(array('bp.*', 'bpcr.`relation_post_id`', 'bpcr.`relation_category_id`'));
		//echo $srch->getQuery();
		$rs = $srch->getResultSet();
		$this->total_records = $srch->recordCount();
		$this->total_pages = $srch->pages();
	...
	function setMainImage($img_id, $blog_post_id){
		...
		if($this->db->startTransaction() && $this->db->update_from_array('tbl_blog_post_images', array('post_image_default'=>0), array('smt'=>'`post_image_post_id`=? AND `post_image_default`=?', 'vals'=>array($blog_post_id,1))) && $this->db->update_from_array('tbl_blog_post_images', array('post_image_default'=>1), array('smt'=>'`post_image_id`=? AND `post_image_post_id`=?', 'vals'=>array($img_id, $blog_post_id))) && $this->db->rows_affected() == 1 && $this->db->commitTransaction()){
			return true;
		}
	...
	function getPostImages($blog_post_id){
		...
		$srch->addOrder('post_image_default','DESC');
		$rs = $srch->getResultSet();
		$pimgs = array();
	...
	function deletePost($post_id, $relation_category_id){
		...
		if($this->db->deleteRecords('tbl_blog_post', array('smt'=>'post_id=?', 'vals'=>array($post_id)), array(), '', 1) && $this->db->deleteRecords('tbl_blog_post_category_relation', array('smt'=>'`relation_post_id`=? AND `relation_category_id`=?', 'vals'=>array($post_id, $relation_category_id)))){
			return true;
		}
		$this->error = $this->db->getError();
		return false;
